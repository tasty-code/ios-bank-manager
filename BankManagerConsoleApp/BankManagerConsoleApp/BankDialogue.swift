...
enum BankDialogue: CustomStringConvertible {
    var description: String {
        ...
        case .input:
            "입력 : "
        case .start(let customer, let task):
            "\(customer) \(task.name)업무 시작"
        case .finish(let customer, let task):
            "\(customer) \(task.name)업무 완료"
    ...
    case menu
    case input
    case start(_ customer: Customer, task: BankTask)
    case finish(_ customer: Customer, task: BankTask)
